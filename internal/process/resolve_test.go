...
func TestResolveCommand_BatchFilesRejected(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !strings.Contains(err.Error(), "specify native interpreter") && !strings.Contains(err.Error(), "native interpreter") {
				t.Errorf("error message should advise specifying native interpreter, got: %v", err)
			}
		})
	}
}
