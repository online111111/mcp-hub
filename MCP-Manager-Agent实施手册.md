...
本文面向编码 Agent、运维 Agent 和人工维护者，描述当前 v0.4.0 release candidate 的维护、验证、部署、改名迁移和发布规则。
...
当前基线：Go 1.25+、MCP Go SDK v1.7.0、产品名 MCP Manager、正式 CLI `mcp-manager`、单 Go 二进制、Browser Admin + Remote Admin + `mcp-manager-deployer` Skill。
...
产品原名 MCP Hub。v0.4 迁移中：

- 新 CLI/二进制：`mcp-manager`
...
正式 v0.4.0 前：final main CI green、文档与 final head 一致、依赖 PR 有明确决定、GitHub 仓库 slug 迁移完成或有明确兼容说明、精确 `v0.4.0` tag、Release workflow 成功、六套 `mcp-manager-*` 包和 `SHA256SUMS` 齐全。
